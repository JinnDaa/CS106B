...
// This function re-arranges the Vector elements into increasing order by priority.
...
// This function returns the k elements from that data stream that have the highest weight,
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    // read data from stream and store in a pqarray
    //PQArray pq;
    PQHeap pq;
    DataPoint cur;
    while(stream >> cur){
    ...
    // construct Vector of size k
    Vector<DataPoint> result(k);
    // keep the k elements with the highest weight and remove others in the pqarray
    while(pq.size() > k){
        pq.dequeue();
    }
    // place th elements into result Vector with increasing order by priority
    for (int i = 0; !pq.isEmpty(); i++) {
...
PROVIDED_TEST("topK: time trial with k fixed to 10") {
...
STUDENT_TEST("topK: time trial with n fixed to 200000") {
    int n = 200000;

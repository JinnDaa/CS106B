 ...
 */
const string kYourName = "JINDA";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "SEESENNSSWWSEEWWWN";
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    ...
    while(!needs.isEmpty()){
        // return false if not all needs are found at the end of moves
        if(index == moves.size()){
            return false;
        }

        // get the next node from moves
        if(moves.at(index) == 'N'){
            next = start->north;
        }
        else if(moves.at(index) == 'S'){
            next = start->south;
        }
        else if(moves.at(index) == 'E'){
            next = start->east;
        }
        else if(moves.at(index) == 'W'){
            next = start->west;
        }
        else{
            error("Invalid moves input!");
            return false;
        }

        // return false if moves try to access an invalid link
        if(next == nullptr){
            return false;
        }

        // remove the found item from Set,
        //contains(): O(logN), remove(): O(logN)
        if(needs.contains(next->contents)){
